...
### Phase 7.2: デスクトップ通知（Week 2）⬅️ 次の実装

**優先度:** HIGH
**工数:** 3-4日
**ステータス:** 設計完了 → 実装準備完了

**関連ドキュメント:** [DESKTOP_NOTIFICATION_SPEC.md](./DESKTOP_NOTIFICATION_SPEC.md)
...
### Phase 7.4: 繰り返しタスク機能拡張（Week 5-9）⏸ 優先度低下

**優先度:** MEDIUM（Phase 7.2-7.3完了後に実施）
**工数:** 5週間（段階的実装）
**ステータス:** 仕様策定完了 → Phase 7.2-7.3完了後に実装

**関連ドキュメント:** [RECURRING_TASK_SPEC.md](./RECURRING_TASK_SPEC.md)

**優先度変更理由:**
- デスクトップ通知とダークモードを優先
- 繰り返しタスクは既存機能で最低限動作している
- データ互換性は確認済み、後から実装可能

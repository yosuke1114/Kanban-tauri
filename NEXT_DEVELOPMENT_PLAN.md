...
**現状問題**:
- `ColumnManager.tsx`にGripVerticalアイコンがあるが、ドラッグ&ドロップが機能していない
- ユーザーが列の順序を変更できない

**実装内容**:
```typescript
// ColumnManager.tsx に@dnd-kitを統合
- SortableContext + useSortable フックを使用
- KanbanBoardと同じパターンで実装
- reorderColumns アクションは既に useBoardStore に実装済み
```

**影響範囲**: `src/components/kanban/ColumnManager.tsx`

**完了基準**:
- ✅ 列をドラッグ&ドロップで並べ替えられる
- ✅ 並べ替えがlocalStorageに保存される
- ✅ ビジュアルフィードバックが表示される
- ✅ テスト追加（ColumnManager.test.tsx）
...
- タスク追加フォームをヘッダーに移動（既に実装済み ✅）
- Apple Reminders風のクリーンなUI
...
| 日 | タスク | 見積もり |
|----|--------|---------|
| 2/1 | 開発計画作成、ドキュメント更新 | 2h |
| 2/2 | 列の並べ替え機能実装 | 4h |
| 2/3 | タイトル重複解消 | 2h |
| 2/4-5 | 共通定数の抽出 | 6h |
| 2/6-7 | 共有コンポーネント作成 | 8h |
